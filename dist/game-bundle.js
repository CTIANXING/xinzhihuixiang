...
__export(types_exports, {
  DEFAULT_FORMAT_PROMPT: () => DEFAULT_FORMAT_PROMPT,
  DEFAULT_OPAQUE_TAGS: () => DEFAULT_OPAQUE_TAGS,
...
var DEFAULT_FORMAT_PROMPT, DEFAULT_TAGS, DEFAULT_OPAQUE_TAGS, DEFAULT_SETTINGS, DEFAULT_PROMPT_ORDER;
var init_types = __esm({
  "src/sillytavern/types.ts"() {
    "use strict";
    DEFAULT_FORMAT_PROMPT = `\u4F60\u5FC5\u987B\u4E25\u683C\u6309\u7167\u4EE5\u4E0B XML \u6807\u7B7E\u683C\u5F0F\u8F93\u51FA\u56DE\u590D\uFF0C\u4E0D\u8981\u4F7F\u7528 Markdown \u5305\u88F9\uFF1A
...
<vars>{ "\u91D1\u94B1": +10, "HP": 38 }</vars>   \u2190 \u9009\u586B\uFF1BJSON \u6DF1\u5408\u5E76`;
    DEFAULT_TAGS = ["maintext", "option", "sum", "vars", "thinking", "think"];
    DEFAULT_OPAQUE_TAGS = ["thinking", "think"];
    DEFAULT_SETTINGS = {
      api: {
...
var DB_VERSION = 3;
var AppDatabase = class extends import_wrapper_default {
  constructor() {
    super(DB_NAME);
    ...
    this.version(3).stores({
      ...
      for (const s of settings) {
        if (s.uiMode === void 0) s.uiMode = "game";
        if (s.customTags === void 0) s.customTags = ["maintext", "option", "sum", "vars", "thinking", "think"];
        if (s.thinkingDisplay === void 0) s.thinkingDisplay = "fold";
        if (s.formatPromptTemplate === void 0) s.formatPromptTemplate = "";
        if (s.api && s.api.secondary === void 0) {
...
async function initializeDatabase() {
  ...
  if (presetCount === 0) {
    const { createDefaultPreset: createDefaultPreset2 } = await Promise.resolve().then(() => (init_types(), types_exports));
    const defaultPreset = createDefaultPreset2();
...
async function saveLorebook(lorebook) {
...
var LorebookEngine = class {
  ...
  scan(text, additionalContext) {
    ...
    const normalizedContext = additionalContext ? this.lorebook.caseSensitive ? additionalContext : additionalContext.toLowerCase() : normalizedText;
    const matched = [];
    for (const entry of this.lorebook.entries) {
      ...
      if (isMatch) {
        matched.push({
          entry,
          score: entry.order,
          matchedKeywords: entry.keys.filter(
            (k) => this.containsKeyword(normalizedText, this.normalizeKeyword(k))
          )
        });
      }
    }
  ...
  recursiveScan(initialText, maxDepth = 3, additionalContext) {
    ...
    const allMatched = /* @__PURE__ */ new Map();
  ...
  checkEntryMatch(entry, text, context) {
    ...
    const secondaryMatches = secondaryKeys.map(
      (k) => this.containsKeyword(context, this.normalizeKeyword(k))
    );
...
function applyParsedToChat(current, parsed) {
...
function assemblePrompt(options) {
  ...
  const systemPrompt = assembledMessages.filter((m) => m.role === "system").map((m) => m.content).join("\n\n");
  return {
...
function replaceMacros(template, context) {
  let result = template.replace(/\{\{user\}\}/g, context.userName).replace(/\{\{char\}\}/g, context.characterName).replace(/\{\{original\}\}/g, context.userInput);
...
function createApiRouter(settings, deps = {}) {
  ...
  async function call(task, payload) {
    ...
    if (target === "secondary") {
      try {
        const res2 = await callOnce("secondary", payload);
        if (!res2.ok) throw new Error(`secondary HTTP ${res2.status}`);
        return { targetUsed: "secondary", response: res2 };
      } catch {
        const res2 = await callOnce("primary", payload);
        return { targetUsed: "primary", response: res2 };
      }
    }
...
// src/sillytavern/api-tools.ts
var COMMON_MODELS_BY_HOST = [
...
var FALLBACK_MODELS = ["gpt-3.5-turbo", "gpt-4", "deepseek-chat", "qwen-turbo"];
...
// src/sillytavern/editor-utils.ts
var ENTRY_DEFAULTS = {
  keys: [],
  secondaryKeys: [],
  content: "",
...
function applyEntryDefaults(partial) {
  return {
...
function createDefaultLorebook(name) {
  const now = Date.now();
  return {
    id: crypto.randomUUID(),
    name,
...
function updateEntry(book, entryId, patch) {
...
function movePromptItem(arr, from, to) {
...
// src/vanilla/sillytavern-store.ts
function createSillytavernStore() {
  let lorebooks = [];
  let presets = [];
  let settings = null;
  ...
  const toggleLorebook = async (id) => {
    const newIds = activeLorebookIds.includes(id) ? activeLorebookIds.filter((i) => i !== id) : [...activeLorebookIds, id];
  ...
  const sendMessage = async (content) => {
    ...
    try {
      ...
      if (activePreset.settings.openai_max_tokens !== void 0) requestBody.max_tokens = activePreset.settings.openai_max_tokens;
      if (activePreset.settings.top_p_openai !== void 0) requestBody.top_p = activePreset.settings.top_p_openai;
...
var sillytavernStore = createSillytavernStore();
